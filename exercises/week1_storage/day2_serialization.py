...
Problem: Convert time-series data to/from JSON format with proper type handling

Learning Objectives:
- Understand serialization trade-offs (JSON vs binary)
- Handle different field types correctly
- Design flexible schema
- Ensure data type preservation

Real-World Connection:
InfluxDB uses line protocol for writes but stores data in binary TSM format.
Your task is to implement JSON serialization that preserves data types.
"""
...
def test_serialization():
    """
    Test cases for serialization functionality.
    """
    print("Testing Time-Series Serialization...")

    # Test 1: Basic DataPoint creation and serialization
    timestamp = time.time()
    tags = {"host": "server1", "region": "us-west", "environment": "production"}
    fields = {
        "cpu_usage": 75.5,  # float
        "memory_usage": 85,  # integer
        "hostname": "web-server-01",  # string
        "is_healthy": True,  # boolean
    }

    point = DataPoint("system", timestamp, tags, fields)
    ...
    assert point_dict["fields"]["cpu_usage"]["type"] == "float"
    assert point_dict["fields"]["memory_usage"]["type"] == "integer"
    print("✓ Test 1 passed: DataPoint serialization")

    # Test 2: Round-trip serialization
    restored_point = DataPoint.from_dict(point_dict)
    assert restored_point.measurement == "system"
    assert restored_point.timestamp == timestamp
    assert restored_point.tags["host"] == "server1"
    assert restored_point.fields["cpu_usage"] == 75.5
    assert isinstance(restored_point.fields["memory_usage"], int)
    assert isinstance(restored_point.fields["is_healthy"], bool)
    print("✓ Test 2 passed: Round-trip serialization")

    # Test 3: Line protocol generation
    line_protocol = point.to_line_protocol()
    assert "system" in line_protocol
    assert "host=server1" in line_protocol
    assert "cpu_usage=75.5" in line_protocol
    assert "memory_usage=85i" in line_protocol, "Integer field should have 'i' suffix"
    assert "is_healthy=true" in line_protocol, "Boolean should be lowercase"
    assert line_protocol.endswith(
        str(int(timestamp * 1e9))
    ), "Should end with timestamp in nanoseconds"
    print("✓ Test 3 passed: Line protocol generation")
    ...
    points = []
    for i in range(5):
        point_tags = {"host": f"server{i%2+1}", "region": "us-west"}
        point_fields = {"cpu": 50.0 + i * 10, "requests": 100 + i}
        points.append(
            DataPoint("http_metrics", timestamp + i, point_tags, point_fields)
        )

    # Serialize batch
    batch_json = serializer.serialize_batch(points)
    batch_data = json.loads(batch_json)

    assert batch_data["count"] == 5
    assert "checksum" in batch_data
    assert len(batch_data["data_points"]) == 5
    print("✓ Test 4 passed: Batch serialization")

    # Test 5: Batch deserialization
    restored_points = serializer.deserialize_batch(batch_json)
    assert len(restored_points) == 5
    assert restored_points[0].measurement == "http_metrics"
    assert restored_points[2].fields["cpu"] == 70.0
    print("✓ Test 5 passed: Batch deserialization")

    # Test 6: Field type detection
    assert point.get_field_type("cpu_usage") == FieldType.FLOAT
    assert point.get_field_type("memory_usage") == FieldType.INTEGER
    assert point.get_field_type("hostname") == FieldType.STRING
    assert point.get_field_type("is_healthy") == FieldType.BOOLEAN
    print("✓ Test 6 passed: Field type detection")

    print("\n🎉 All serialization tests passed!")
    print("Your serialization system correctly handles:")
    print("  - Type preservation across serialization")
    print("  - Batch operations with metadata")
    print("  - Line protocol generation")
...
if __name__ == "__main__":
    """
    Instructions:
    1. Implement all TODO methods in DataPoint and TimeSeriesSerializer classes
    2. Run this file to test: python day2_serialization.py
    3. All tests should pass
    4. Try creating your own test data points

    Success criteria:
    - All 6 tests pass
    - Type information is preserved during serialization
    - Line protocol format matches InfluxDB specification
    - Batch operations work correctly
    - Checksums provide data integrity

    Next steps:
    - Move to day3_line_protocol.py
    - Think about: How would you optimize for very large batches?
    - Consider: What other serialization formats could work? (MessagePack, Protocol Buffers)
    """
    test_serialization()
...
1. Serialization Trade-offs:
   - JSON: Human readable, widely supported, but larger size
   - Binary: Compact, fast, but not human readable
   - Type preservation: Critical for numerical accuracy

2. Schema Design:
   - Flexible schema allows evolution
   - Type information prevents data corruption
   - Metadata enables validation and debugging

3. Batch Processing:
   - Amortizes overhead across multiple operations
   - Enables compression optimizations
   - Improves throughput for high-volume data

4. Data Integrity:
   - Checksums detect corruption
   - Version numbers enable compatibility checks
   - Validation prevents invalid data entry

Connection to InfluxDB:
- InfluxDB line protocol optimizes for write speed
- TSM files use binary format for storage efficiency
- Field types are strictly enforced
- Batch writes improve performance

Performance Implications:
- JSON parsing overhead vs binary deserialization
- Memory usage during large batch operations
- CPU cost of checksum calculation
- Network bandwidth for different formats
